...
public class ZEDMixedRealityPlugin : MonoBehaviour
{
	...
	public void UpdateRenderPlane()
	{
        ...
        if (finalLeftEye != null) //   // #KV: Changed this to support VRTK's dynamic loading. Added null check for finalLeftEye
        {
	...
	public void LateUpdateHmdRendering()
	{
		...
		{
			if (leftScreen != null && leftScreen.target != null && leftScreen.target.IsCreated())  // #KV: Changed this to support VRTK's dynamic loading. Added null check for leftScreen
            {
				leftMaterial.SetTexture("_MainTex", leftScreen.target);
				ready = true;
			}
			else ready = false;
			if (rightScreen != null && rightScreen.target != null && rightScreen.target.IsCreated())  // #KV: Changed this to support VRTK's dynamic loading. Added null check for rightScreen
            {
